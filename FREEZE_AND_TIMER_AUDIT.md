...
# Round 2 — end-to-end lifecycle audit (2026-08-03, after the Figma Make push)

**Context.** The Round 1 branch was merged (PR #1) and a Figma Make push landed
on top. Verified against `origin/main`: `functions/src/index.ts`,
`src/lib/submissionService.ts`, both test files and `firestore.rules` survived
byte-identical; the push deleted the `.gitignore`s, this document,
`functions/package-lock.json` and the compiled `lib/` artifacts (harmless —
the predeploy hook rebuilds `lib/` from `src/`). Both suites passed on
`origin/main` unchanged.

This round ran the whole lifecycle — creation → publish → start → sitting →
break → freeze → finalisation → grading — through a third executable suite:

```
npm test              # all three
npm run test:e2e      # functions/test/exam.e2e.cjs — 10 scenarios, 85 checks, PASS
```

## New findings

### G1 · Finalisation never cleared live freeze state — and the roster shows `frozenAt` over a terminal status — **S1, and pre-existing**
`AssessmentRosterCore.tsx` (`deriveRosterStatus`) checks `attempt.frozenAt`
**before** the terminal status, and no finalisation path — `gradeAttempt`, the
sweep, either — ever deleted `frozenAt` or closed the open ledger entry. Any
sitting that ended while paused therefore shows as **"frozen" in the roster
forever**, over a status of `submitted`/`auto_submitted`/`terminated`.

This defect predates Round 1 entirely: under the pre-fix deployed code a
paused student could self-submit (old F5), which left exactly this state
behind. **It is the most likely explanation for "the freeze problem still
exists / things are not adding up" as seen from the roster — including on
attempts frozen and finished weeks ago, which no functions deploy will
retroactively clean.**

Fixed threefold: `gradeAttempt` and the sweep now close the open ledger entry
(zero grant), delete `frozenAt`/`frozenBy`/`frozenReason` and stamp
`freezeState` cleared at finalisation; and `deriveRosterStatus` now lets a
terminal status win over a leftover `frozenAt`, so historical dirty attempts
display truthfully with no backfill. Proof: `E2E-05` … `E2E-08`.

### G2 · Round 1's ATTEMPT_PAUSED gate broke the trapped-frozen escape hatch — **S1, introduced by Round 1**
The F5 gate refused a student finalise for *any* open freeze — including one
whose availability window had closed, which is the one case D-02/D8 built the
escape hatch for. Worse, the old escape test (`attemptWindowClosed`) reads
`answersLockedAfter` — the very lock an open freeze *holds* — so it contradicted
the pause semantics both ways. Both decisions now go through the resolver: an
open freeze blocks finalise **unless `resolve()` says the sitting is ended**
(window shut in real time, or over before the pause began). Proof: `E2E-07`.

### G3 · The flush-on-freeze was denied by rules in standard delivery — **S1, introduced by Round 1**
F5 made a pause set `status:'frozen'`; the student update rule required
`in_progress` on both sides; so the one write that exists to save the answer
in front of a student being paused was refused in standard mode. The rule now
accepts `'frozen'` on the student answer path — the same posture
`saveAnswerNoAdvance` documents — still bounded by the field whitelist and
`answerWriteWindowOpen`. (Rules fix; verified by inspection — no rules
harness exists.)

### G4 · An attempt frozen after it was already over could never be closed — **S2**
The sweep's frozen branch only acted on `window_closed`. An attempt paused
*after* its overall clock ran out — with no `endDate` — was invisible to both
sweep queries forever. The branch now closes on **any** `ended` verdict, which
with the freeze pinning evaluation time means exactly: the window shut, or the
sitting was over before the pause began. Proof: `E2E-08`.

### G5 · INV-2 / INV-9 fired falsely on every standard-mode sitting — **S3, checker noise**
`checkInvariants` applied "at most one unlocked served question" and "served
before its section started" unconditionally, but standard delivery serves the
entire paper unlocked at `startExam` **by design**. The Phase 3b production
shadow has been logging phantom violations on every standard sitting, burying
real signal. Both checks are now scoped to sequential delivery; the sweep's
INV-2 regression asserts both directions. Proof: `E2E-01`, sweep regression.

## Verified sound this round (not bugs)

* The unfreeze modal's units: seconds in the UI, converted to ms at submit
  (`grantNum * 1000`, penalties likewise) — `AssessmentRosterCore.tsx:665-668`.
* Break machinery end-to-end: client `endsAt = submittedAt + duration +
  breakCreditMs`, matching `pendingBreak`; `getBreakState` (fixed Round 1);
  the mandatory-break gate with credit — `E2E-10`.
* `expiredClock` reads the materialised (credited) locks.
* `evaluateStudentAttempts`: frozen counts as live; retake/limit logic exact —
  `E2E-03`.
* Availability gates incl. D-11 (hand-closed blocks new, resumes live) —
  `E2E-04`.
* Grading is deterministic across finalisation paths — `E2E-09`.
* The scorer's shape: `scores.{total, available, percentage, passed}`;
  assessment-level floor at zero; blank ≠ wrong.

## Deploy checklist (nothing takes effect until this is done)

```bash
cd functions && npm install && npm test     # expect: three green suites
firebase deploy --only functions
firebase deploy --only firestore:rules
```

Vercel picks up the client change (`AssessmentRosterCore.tsx`) on push
automatically. The roster fix displays historical stuck-frozen attempts
correctly even before the functions deploy.
