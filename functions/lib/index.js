...
exports.submitSection = exports.startSection = exports.startExam = exports.getServerTime = exports.gradeAttempt = exports.deleteAuthUser = exports.createAuthUser = void 0;
...
exports.gradeAttempt = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    if (!request.auth)
        throw new https_1.HttpsError('unauthenticated', 'Sign-in required.');
    const callerRole = request.auth.token.role;
...
exports.startExam = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    if (!request.auth)
        throw new https_1.HttpsError('unauthenticated', 'Sign-in required.');
    ...
    const { assessmentId, sections, shuffleQuestions, sectionStartOrder, cameraDeclined } = request.data || {};
    if (!assessmentId || !Array.isArray(sections) || sections.length === 0) {
        throw new https_1.HttpsError('invalid-argument', 'assessmentId and sections are required.');
    }
    const db = (0, firestore_1.getFirestore)();
    ...
    const live = attemptsSnap.docs.find((d) => d.data().status === 'in_progress'
        || d.data().status === 'frozen');
    if (live)
        return { ok: true, attempt: live.data() };
    const effectiveMax = a.attemptOverrides?.[studentId] ?? a.maxAttempts ?? 1;
    const finished = attempts.filter((t) => t.status === 'submitted' || t.status === 'auto_submitted' || t.status === 'terminated').length;
    if (finished >= effectiveMax) {
        throw new https_1.HttpsError('resource-exhausted', `ATTEMPT_LIMIT_EXCEEDED:${finished}:${effectiveMax}`);
    }
...
exports.startSection = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    if (!request.auth)
        throw new https_1.HttpsError('unauthenticated', 'Sign-in required.');
    const role = request.auth.token.role;
    const studentId = request.auth.token.studentId;
...
exports.submitSection = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    if (!request.auth)
        throw new https_1.HttpsError('unauthenticated', 'Sign-in required.');
    const role = request.auth.token.role;
    const studentId = request.auth.token.studentId;
