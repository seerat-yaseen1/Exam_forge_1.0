...
exports.purgeAttemptChildRecords = purgeAttemptChildRecords;
exports.attemptIdsForStudent = attemptIdsForStudent;
...
const JUDGE0_BASE_URL = (0, params_1.defineString)('JUDGE0_BASE_URL', { default: '' });
const JUDGE0_AUTH_TOKEN = (0, params_1.defineSecret)('JUDGE0_AUTH_TOKEN');
/**
...
const judge0Adapter_1 = require("./judge0Adapter");
 ...
 */
async function purgeAttemptChildRecords(db, attemptIds, opts) {
...
async function attemptIdsForStudent(db, studentId) {
    try {
...
exports.executeErasure = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    ...
    if (subjectRole === 'student') {
        ...
        if (child.verdictsDeleted > 0)
            counts.verdictsDeleted = child.verdictsDeleted;
...
function resolveGradingPolicyS(config, sectionId, difficulty, isCoding = false) {
 ...
 */
function codeVerdictDocId(attemptId, questionId) {
 ...
 */
async function loadCodeVerdicts(db, attemptId, questionMap) {
    const out = new Map();
    ...
    const qIds = [];
    questionMap.forEach((q, id) => { if (q.engine === 'code')
        qIds.push(id); });
    if (qIds.length === 0)
        return out;
...
function getJudgeAdapter() {
    ...
    if (!baseUrl) {
        ...
        console.error('[judge] JUDGE0_BASE_URL is not set — no judge is configured, so every '
    ...
    judgeAdapterInstance = new judge0Adapter_1.Judge0Adapter({ baseUrl, authToken });
    return judgeAdapterInstance;
}
/**
...
function scoreAttemptAnswers(params) {
    ...
    const codeVerdicts = params.codeVerdicts ?? new Map();
        ...
        for (const aq of sec.questions) {
            ...
            if (invalidated.has(aq.questionId)) {
                ...
                if (q.engine === 'mcq') {
                    ...
                    const outcome = verdict ? (0, judgeCore_1.outcomeFor)(verdict, ans.tests ?? []) : null;
                    if (outcome) {
                        const award = awardFor(outcome, policy, aq.marks);
                        sectionAwarded += award;
                        totalAwarded += award;
...
exports.regradeAttempts = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    ...
    const answerMap = new Map();
    {
        const qIds = Array.from(new Set(liveSections.flatMap((s) => s.questions.map((q) => q.questionId))));
        const loaded = await loadQuestionAndAnswerMaps(db, qIds);
        for (const [k, v] of loaded.questionMap)
            questionMap.set(k, v);
        for (const [k, v] of loaded.answerMap)
            answerMap.set(k, v);
    }
    ...
    for (const docSnap of attemptsSnap.docs) {
        ...
        const attemptPaper = examContractFor(att, assessment);
        const sections = normalizeSections(attemptPaper);
        const missing = Array.from(new Set(sections.flatMap((sec) => sec.questions.map((q) => q.questionId)))).filter((qid) => !questionMap.has(qid));
        if (missing.length > 0) {
            const extra = await loadQuestionAndAnswerMaps(db, missing);
            for (const [k, v] of extra.questionMap)
                questionMap.set(k, v);
...
exports.getAnswerKeysForReview = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    ...
    try {
        ...
        for (const d of satSnap.docs) {
            const snapSections = d.get('examSnapshot')?.sections;
            if (!Array.isArray(snapSections))
                continue;
            for (const sec of snapSections) {
                for (const q of sec.questions ?? []) {
                    if (typeof q.questionId === 'string')
                        paperIds.add(q.questionId);
                }
            }
        }
    }
...
exports.getExamQuestions = (0, https_1.onCall)(EXAM_HOT_PATH, async (request) => {
    ...
    const paperForAttempt = liveAttempt
        ? examContractFor(liveAttempt, assessment)
        : assessment;
    const qIds = isSequentialDelivery
        ? Array.from(new Set((liveAttempt?.servedQuestions ?? []).map((s) => s.questionId)))
        : Array.from(new Set(normalizeSections(paperForAttempt).flatMap((s) => s.questions.map((q) => q.questionId))));
...
exports.submitAnswerAndAdvance = (0, https_1.onCall)(EXAM_HOT_PATH, async (request) => {
    ...
    const contractForQ = assessment
        ? examContractFor(attempt, assessment)
        : undefined;
    ...
    const lateGraceSec = contractForQ?.questionGraceSeconds
...
EXAM_HOT_PATH, async (request) => {
    ...
    // reads.
    auditTiming('saveAnswerNoAdvance', attemptId, attempt, assessment, ['question', 'section', 'break', 'choose', 'ended', 'not_started']);
...
exports.freezeAttempt = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    ...
    if (!result.alreadyFrozen && result.actor) {
        ...
        // the whole point.
        await auditTimingFromStore(db, 'freezeAttempt', attemptId, ['question', 'section', 'break', 'choose', 'ended', 'not_started']);
    }
...
exports.unfreezeAttempt = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    ...
    await auditTimingFromStore(db, 'unfreezeAttempt', attemptId, ['question', 'section', 'break', 'choose', 'ended', 'not_started']);
    return {
        ok: true,
 ...
 * never fail a freeze.
 */
async function auditTimingFromStore(db, where, attemptId, decided) {
    try {
        const aSnap = await db.collection('attempts').doc(attemptId).get();
        if (!aSnap.exists)
            return;
        const attemptRaw = aSnap.data();
        const assessmentId = attemptRaw.assessmentId;
        if (!assessmentId)
            return;
        const asmtSnap = await db.collection('assessments').doc(assessmentId).get();
        if (!asmtSnap.exists)
            return;
        auditTiming(where, attemptId, attemptRaw, asmtSnap.data(), decided);
    }
    catch (e) {
...
exports.runCodeSample = (0, https_1.onCall)({ region: 'us-central1', secrets: [JUDGE0_AUTH_TOKEN] }, async (request) => {
    ...
    if (verdict.status === 'judge_unavailable' || verdict.status === 'internal_error') {
        console.error(`[judge] sample run unavailable for attempt=${attemptId} question=${questionId} `
