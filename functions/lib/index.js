...
exports.scheduledCloseExpiredAttempts = (0, scheduler_1.onSchedule)({
    ...
    for (const doc of frozenSnap.docs) {
        const d = doc.data();
        if (!attemptWindowClosed(d, nowMs))
            continue;
        candidates.push({
            doc,
            reason: 'deadline_expired_sweep',
...
exports.deleteAuthUser = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    ...
    const callerRole = request.auth.token.role;
    const callerInstituteId = request.auth.token.instituteId;
    const { role, uid, successorId, confirmLiveOwnership, deleteAttemptsOnWebOwnerAssessments } = request.data || {};
    if (!role || !COLLECTION_BY_ROLE[role])
        throw new https_1.HttpsError('invalid-argument', 'Invalid role.');
    if (!uid)
        throw new https_1.HttpsError('invalid-argument', 'uid is required.');
...
exports.gradeAttempt = (0, https_1.onCall)({ region: 'us-central1', secrets: [SEB_SIGNING_SECRET] }, async (request) => {
    ...
    const isTrappedFrozen = attempt.status === 'frozen' && attemptWindowClosed(attempt);
...
exports.verifyAndResume = (0, https_1.onCall)({ region: 'us-central1', secrets: [SEB_SIGNING_SECRET] }, async (request) => {
    ...
    await ref.update({
        ...
        ...(frozenForSeconds > 0
            ? { totalFrozenSeconds: firestore_1.FieldValue.increment(frozenForSeconds) }
            : {}),
        updatedAt: nowIso,
    });
...
exports.submitAnswerAndAdvance = (0, https_1.onCall)(EXAM_HOT_PATH, async (request) => {
    ...
    if (typeof qLimit === 'number' && qLimit > 0) {
        ...
        if (elapsedSec > qLimit + 5)
            lateAnswer = true; // 5s grace for latency
    }
...
exports.freezeAttempt = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    ...
    const result = await db.runTransaction(async (txn) => {
        ...
        try {
            ...
            if (aSnap?.exists) {
                const core = toCoreAttempt(att);
                const asmt = toCoreAssessment(aSnap.data());
                const d = (0, examTimingCore_1.computeDeadlines)(core, asmt);
                ...
                const left = (x) => x === null ? null : Math.max(0, x - nowMsF);
                clocksAtFreeze = {
        ...
        txn.update(ref, {
            freezes: firestore_1.FieldValue.arrayUnion(entry),
...
exports.unfreezeAttempt = (0, https_1.onCall)({ region: 'us-central1' }, async (request) => {
    ...
    const result = await db.runTransaction(async (txn) => {
        ...
        const closed = {
            ...(idx >= 0 ? ledger[idx] : {
                id: `fz_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`,
                startedAt: startedAtIso, reason: 'invigilator',
            }),
            endedAt: nowIso,
            elapsedMs,
            grantedMs: granted,
        ...
        const openId = (0, examTimingCore_1.openSectionId)(creditedAttempt);
        ...
        const dl = (0, examTimingCore_1.computeDeadlines)(creditedAttempt, lockA);
        const remaining = (endsAt) => endsAt === null ? 0 : Math.max(0, endsAt - nowMsUnfreeze);
        const wanted = request.data?.penalties ?? {};
        const clamp = (askedMs, capMs) => {
        ...
        const addPenalty = (clock, amountMs) => {
            if (amountMs <= 0)
                return; // no row for a decision not taken
            penaltyRows.push({
        ...
        applyLockUpdates(updates, computeAttemptLocks(att.startedAt, openSectionIso, openSectionLimit, lockA, creditedAttempt));
...
creditFrom) {
    ...
    const secMs = (0, examTimingCore_1.sectionDeadlineMs)(sectionStartedAtIso, sectionTimeLimitMin, a.sectionGraceSeconds, sectionCredit);
    const ovrMs = (0, examTimingCore_1.overallDeadlineMs)(attemptStartedAtIso, a.overallTimeLimit, a.overallGraceSeconds, overallCredit);
    ...
    if (idx > 0) {
        ...
        if (prevTiming?.submittedAt) {
            ...
            if (brk && brk.mandatory) {
                const breakEndsAt = new Date(prevTiming.submittedAt).getTime() + brk.durationMinutes * 60000;
...
exports.submitSection = (0, https_1.onCall)(EXAM_HOT_PATH, async (request) => {
    ...
    if (a.overallTimeLimit && a.overallTimeLimit > 0 && attempt.startedAt) {
        ...
        const overallDeadlineMs = examStartMs + a.overallTimeLimit * 60000 + overallGraceSec * 1000;
        if (serverNow > overallDeadlineMs) {
            ...
            if (sec?.timeLimit && sec.timeLimit > 0) {
                const sectionDeadlineMs = startedMs + sec.timeLimit * 60000 + graceSec * 1000;
    ...
    if (sec?.timeLimit && sec.timeLimit > 0) {
        const deadlineMs = startedMs + sec.timeLimit * 60000 + graceSec * 1000;
