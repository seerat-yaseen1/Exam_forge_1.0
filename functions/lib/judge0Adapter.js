...
exports.Judge0Adapter = exports.J0 = exports.JUDGE0_LANGUAGE_NAMES = exports.JUDGE0_LANGUAGE_IDS = exports.JUDGE0_IMAGE = void 0;
exports.verifyLanguageTable = verifyLanguageTable;
exports.isTerminal = isTerminal;
exports.mapTestStatus = mapTestStatus;
exports.buildJudge0Submission = buildJudge0Submission;
exports.verdictFromResults = verdictFromResults;
exports.httpTransport = httpTransport;
const judgeCore_1 = require("./judgeCore");
// ══════════════════════════════════════════════════════════════════
// §1 — LANGUAGE TABLE
// ══════════════════════════════════════════════════════════════════
/**
 ...
 */
exports.JUDGE0_IMAGE = 'judge0/judge0:1.13.1';
exports.JUDGE0_LANGUAGE_IDS = {
 ...
 */
function verifyLanguageTable(live) {
    const byId = new Map(live.map((l) => [l.id, l.name]));
    const bad = [];
    Object.keys(exports.JUDGE0_LANGUAGE_IDS).forEach((lang) => {
        const id = exports.JUDGE0_LANGUAGE_IDS[lang];
        const name = byId.get(id) ?? null;
        if (name === null || !name.startsWith(exports.JUDGE0_LANGUAGE_NAMES[lang])) {
...
function isTerminal(statusId) {
    return statusId > exports.J0.PROCESSING;
}
/**
 ...
 */
function mapTestStatus(statusId) {
    switch (statusId) {
        case exports.J0.ACCEPTED:
        case exports.J0.WRONG_ANSWER:
            return null; // ran; we compare the output ourselves
        case exports.J0.TIME_LIMIT_EXCEEDED:
            return 'timeout';
        case exports.J0.COMPILATION_ERROR:
            return 'skipped'; // handled at the submission level
        case exports.J0.INTERNAL_ERROR:
        case exports.J0.EXEC_FORMAT_ERROR:
...
const b64decode = (v) => typeof v === 'string' && v.length > 0 ? Buffer.from(v, 'base64').toString('utf8') : '';
const b64encode = (v) => Buffer.from(v, 'utf8').toString('base64');
/**
 ...
 */
function buildJudge0Submission(sub, test) {
    const cpuSec = sub.limits.cpuMs / 1000;
    return {
        language_id: exports.JUDGE0_LANGUAGE_IDS[sub.language],
...
function isInfraStatus(id) {
    return id === exports.J0.INTERNAL_ERROR || id === exports.J0.EXEC_FORMAT_ERROR;
}
/**
 ...
 */
function verdictFromResults(sub, results, adapterName, now = new Date()) {
    const judgedAt = now.toISOString();
    const tests = sub.tests;
    // (1) compile error — submission-level
    const compiled = results.findIndex((r) => r.status?.id === exports.J0.COMPILATION_ERROR);
    if (compiled >= 0) {
        return {
    ...
    if (infra) {
        return (0, judgeCore_1.unavailableVerdict)(adapterName, `Judge0 reported ${infra.status?.description ?? 'an internal error'}`
            + `${infra.message ? `: ${b64decode(infra.message)}` : ''}`, now);
    }
    // (3) per-test
    const out = [];
    for (let i = 0; i < tests.length; i++) {
        ...
        const statusId = r.status?.id ?? exports.J0.INTERNAL_ERROR;
        const mapped = mapTestStatus(statusId);
        const stdout = b64decode(r.stdout);
        const stderr = b64decode(r.stderr);
        const status = mapped !== null
            ? mapped
            : ((0, judgeCore_1.testPasses)(test, stdout) ? 'passed' : 'wrong_answer');
        out.push({
            testId: test.id,
            status,
...
function httpTransport(cfg) {
    ...
    return {
        post: (path, body, timeoutMs) => call(path, { method: 'POST', body: JSON.stringify(body) }, timeoutMs),
...
class Judge0Adapter {
    ...
    async health() {
        try {
            const res = await this.transport.get('/system_info', 5000);
    ...
    async run(sub) {
        ...
        if (this.breakerIsOpen(startMs)) {
            return (0, judgeCore_1.unavailableVerdict)(this.name, 'Judge0 circuit breaker is open.');
        }
        if (sub.tests.length === 0) {
            return (0, judgeCore_1.unavailableVerdict)(this.name, 'Submission carries no tests.');
        }
        try {
            const payload = {
                submissions: sub.tests.map((t) => buildJudge0Submission(sub, t)),
            };
            const created = await this.transport.post('/submissions/batch?base64_encoded=true', payload, 20000);
            if (created.status < 200 || created.status >= 300) {
                this.recordFailure(Date.now());
                return (0, judgeCore_1.unavailableVerdict)(this.name, `Judge0 create returned HTTP ${created.status}`);
            }
            const tokens = Array.isArray(created.json)
                ? created.json.map((s) => s?.token).filter((t) => typeof t === 'string' && t.length > 0)
                : [];
            if (tokens.length !== sub.tests.length) {
                this.recordFailure(Date.now());
                return (0, judgeCore_1.unavailableVerdict)(this.name, `Judge0 accepted ${tokens.length} of ${sub.tests.length} submissions`);
            }
            const results = await this.poll(tokens, startMs);
            if (!results) {
                this.recordFailure(Date.now());
                return (0, judgeCore_1.unavailableVerdict)(this.name, 'Judge0 did not finish within the run deadline.');
            }
            this.recordSuccess();
            ...
            this.recordFailure(Date.now());
            return (0, judgeCore_1.unavailableVerdict)(this.name, `Judge0 request failed: ${e.message}`);
        }
    }
    /** Poll until every submission is terminal, or the deadline passes. */
    async poll(tokens, startMs) {
        ...
        for (let i = 0; Date.now() < deadline; i++) {
            ...
            const res = await this.transport.get(path, 15000);
            if (res.status < 200 || res.status >= 300)
                continue;
            const body = res.json;
            const subs = body?.submissions;
            if (!Array.isArray(subs) || subs.length !== tokens.length)
                continue;
