...
export function FacultyProfilePage() {
  ...
  return (
    ...
    >
      <p className="text-xs mb-1" style={{ color: 'var(--ef-text-muted)', letterSpacing: '0.1em' }}>PROFILE</p>
      ...
      <div style={{ background: 'var(--ef-surface)', border: '1px solid var(--ef-border)', borderRadius: 3 }}>
        {rows.map((row, i) => (
          <div key={i} className="flex items-start px-5 py-4"
            style={{ borderBottom: i < rows.length - 1 ? '1px solid var(--ef-border-subtle)' : 'none' }}>
            <p className="text-xs w-40 flex-shrink-0" style={{ color: 'var(--ef-text-muted)', paddingTop: 1 }}>{row.label}</p>
            <p className="text-sm"
              style={{
                color: 'var(--ef-ink)',
