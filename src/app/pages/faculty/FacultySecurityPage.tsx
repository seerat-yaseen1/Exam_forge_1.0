...
export function FacultySecurityPage() {
  const { changePassword } = useFacultyAuth();
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="mb-6">
            ...
            {confirmPassword && newPassword && (
              <p className="text-xs mt-1.5" style={{ color: confirmPassword === newPassword ? 'var(--ef-success)' : 'var(--ef-danger)' }}>
          ...
          <button type="submit" disabled={!canSubmit}
            className="text-xs px-4 py-2.5 flex items-center gap-1.5 transition-opacity"
            style={{
              background: canSubmit ? 'var(--ef-ink)' : 'var(--ef-track)',
              color: 'var(--ef-surface)', borderRadius: 2, letterSpacing: '0.03em',
              cursor: canSubmit ? 'pointer' : 'not-allowed',
            }}>
            {loading ? <><Loader2 size={11} className="animate-spin" />Saving…</> : 'Update password'}
          </button>
        </form>
      </div>
    </motion.div>
